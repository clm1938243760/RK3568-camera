...
<div id="stage"><div id="viewport"><img id="video" alt="RK3568 camera preview"><canvas id="overlay"></canvas></div></div>
...
function connectVideo(){
  const endpoint="/api/frame.jpg";
  const refresh=()=>{
    video.onload=()=>{
      video.classList.add("playing");
      videoState.textContent="实时";
      frameTimer=setTimeout(refresh,200);
    };
    video.onerror=()=>{
      video.classList.remove("playing");
      videoState.textContent="连接失败";
      frameTimer=setTimeout(refresh,1000);
    };
    video.src=endpoint+"?_="+Date.now();
  };
  refresh();
}
...
function renderStatus(result){
  ...
  if(!result.active){
    if(serviceState==="busy"){
      ...
      setStatus("error","检测服务未运行","服务状态："+detectorUnitState+"，请检查 rk3568-camera-report-trigger");
...
exportText.addEventListener("click",()=>{
  ...
  const link=document.createElement("a");link.href=url;link.download="rk3568-ocr-result.json";link.click();
...
exportPatient.addEventListener("click",()=>{
  ...
  const url=URL.createObjectURL(blob);
  const link=document.createElement("a");link.href=url;link.download="rk3568-patient-result.json";link.click();
  window.setTimeout(()=>URL.revokeObjectURL(url),1000);
});
window.addEventListener("beforeunload",()=>{if(frameTimer)clearTimeout(frameTimer);});
...
def _post_gateway_scan(endpoint: str, identifier: str, timeout_seconds: float) -> None:
    request = Request(
        ...
        headers={"Content-Type": "application/json", "User-Agent": "RK3568-Camera-Identifier"},
        method="POST",
    )
    try:
...
def _post_gateway_patient_query(
    ...
    request = Request(
        endpoint,
        data=json.dumps({"code": identifier}, ensure_ascii=True).encode("utf-8"),
        headers={"Content-Type": "application/json", "User-Agent": "RK3568-Camera-Patient"},
        method="POST",
    )
    try:
        with urlopen(request, timeout=timeout_seconds) as response:
...
class Handler(BaseHTTPRequestHandler):
    server_version = "RK3568CaptureMonitor/0.1"
    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/frame.jpg":
            try:
                request = Request(
                    self.server.frame_endpoint,  # type: ignore[attr-defined]
                    headers={"Accept": "image/jpeg", "User-Agent": "RK3568-Camera-Monitor/0.1"},
                )
                with urlopen(request, timeout=3.0) as response:
                    image = response.read(8 * 1024 * 1024 + 1)
                if len(image) > 8 * 1024 * 1024:
                    raise ValueError("frame_too_large")
                if len(image) < 4 or not image.startswith(b"\xff\xd8") or b"\xff\xd9" not in image[-4096:]:
                    raise ValueError("incomplete_jpeg")
            except (HTTPError, URLError, OSError, ValueError) as exc:
                self._json(
                    {"error": "frame_unavailable:%s" % type(exc).__name__},
...
class Server(ThreadingHTTPServer):
    ...
    def __init__(
        ...
        frame_endpoint: str = "http://127.0.0.1:8090/api/frame.jpg?quality=85",
        page: bytes = PAGE,
    ) -> None:
        ...
        self.frame_endpoint = frame_endpoint
        self.page = page


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="RK3568 report capture monitor")
    parser.add_argument("--host", default="0.0.0.0")
    parser.add_argument("--port", type=int, default=8894)
    parser.add_argument(
        "--frame-endpoint",
        default="http://127.0.0.1:8090/api/frame.jpg?quality=85",
    )
    parser.add_argument(
        "--status-file",
        type=Path,
    ...
    parser.add_argument(
        "--result-file",
        type=Path,
        default=Path("/run/rk3568-camera/verified-identifier.json"),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--rules-file",
        type=Path,
        default=Path("/var/lib/rk3568-camera/active_identifier_rules.json"),
    )
    parser.add_argument(
        "--capture-settings-file",
        type=Path,
        default=Path("/var/lib/rk3568-camera/camera_capture_settings.json"),
    )
    parser.add_argument(
        "--trigger-environment-file",
        type=Path,
        default=Path("/var/lib/rk3568-camera/camera-capture.env"),
    )
    parser.add_argument("--trigger-service", default="rk3568-camera-report-trigger.service")
    ...
    parser.add_argument(
        "--forward-state-file",
        type=Path,
        default=Path("/var/lib/rk3568-camera/identifier_forward_state.json"),
    )
    ...
    parser.add_argument(
        "--patient-result-file",
        type=Path,
        default=Path("/run/rk3568-camera/verified-patient.json"),
    )
    parser.add_argument(
        "--patient-metadata-file",
        type=Path,
        default=Path("/run/rk3568-camera/verified-patient.meta.json"),
    )
    parser.add_argument(
        "--patient-query-state-file",
        type=Path,
        default=Path("/var/lib/rk3568-camera/patient_query_state.json"),
    )
...
def main() -> int:
    args = parse_args()
    frame_url = urlsplit(args.frame_endpoint)
    if frame_url.scheme != "http" or frame_url.hostname not in {"127.0.0.1", "localhost", "::1"}:
        raise ValueError("frame endpoint must be loopback HTTP")
    ...
    print("RK3568 capture monitor listening on http://%s:%d" % (args.host, args.port), flush=True)
