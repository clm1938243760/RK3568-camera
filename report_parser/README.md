...
> RK3568 port note: this directory is imported from the verified RK3588 parser
> baseline. The active RK3568 camera service uses only the text-only capture
> subset with OpenCV DNN and keeps the package namespace for compatibility.
> The root repository README and runtime manifest define the RK3568 deployment.
