...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Low-frequency DocAligner paper trigger for the RK3568 camera adapter"
    )
    parser.add_argument("--frame-glob", default="/tmp/rk3568_camera_ocr_*.jpg")
    parser.add_argument(
        "--paper-model",
        type=Path,
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT_ROOT / "config.rk3568.ocr_only.json",
    )
    parser.add_argument(
        "--rules-file",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("/run/rk3568-camera/configuration-images"),
    )
