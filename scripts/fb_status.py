...
class AssetRenderer:
    ...
    def _medical_page(self, display: dict[str, Any]) -> Image.Image:
        ...
        bbox = text_bbox(draw, title, title_font)
        ...
        ip = "设备 IP：" + board_ip()
        ip_bbox = text_bbox(draw, ip, self.font_ip)
    ...
    def _top_bar(self, draw: ImageDraw.ImageDraw, accent: tuple[int, int, int], tag: str) -> None:
        ...
        badge_w = min(max(text_length(draw, tag, self.font_tiny) + 30, 92), 180)
...
def main() -> int:
    ...
    # RK3568 SPI1 M1 wiring: GPIO3_C4 is DC and GPIO3_B5 is RESET.
    parser.add_argument("--spidev", default="/dev/spidev1.0")
    parser.add_argument("--dc-gpio", type=int, default=116)
    parser.add_argument("--reset-gpio", type=int, default=109)
    parser.add_argument("--cs-gpio", type=int, default=-1)
